        ...
        if "CREATE TABLE" in statement:
            ...
            if table_name_match:
                ...
                for column_name in matches:
                    sequence_declaration, column_declaration = replace_serial(table_name, column_name)
                    sequence_declarations.append(sequence_declaration)
                    statement = re.sub(fr'{column_name}\s+serial', column_declaration, statement, count=1)

                # Add sequence declarations before the CREATE TABLE statement
                processed_statement = "\n".join(sequence_declarations) + "\n" + statement if sequence_declarations else statement
...
def convert_postgres_to_duckdb(sql):
    ...
    sql = re.sub(r'\s+', ' ', sql)  # Replace any whitespace character (space, tab, newline) with a single space
    sql = re.sub(r';\s*', ';\n', sql)  # Ensure semicolons are followed by exactly one newline
...
def sql_to_csv(input_sql_file, output_csv_file, headers = ['tax_id', 'ncbi_taxid', 'superkingdom', 'kingdom', 'phylum', 'class', 'taxonomic_order', 'family', 'genus', 'species', 'strain', 'name']):
...
def init_duckdb_schema(input_sql_dir, output_dir):
    ...
    TABLES = [
        "sampling_sites", "bgc_types", "substrates", "taxa", "profiles", "as_domain_profiles",
        "pfams", "gene_ontologies", "resfams", "tigrfams", "bgc_rules", "samples", "isolates",
        "genomes", "dna_sequences", "regions", "candidates", "protoclusters", "functional_classes",
        "smcogs", "cdss", "genes", "ripps", "t2pks", "monomers", "modules", "as_domains",
        "clusterblast_algorithms", "clusterblast_hits", "tta_codons", "pfam_domains",
        "pfam_go_entries", "filenames", "resfam_domains", "tfbs", "comparippson", "tigrfam_domains",
        "cluster_compare_hits", "rel_candidates_protoclusters", "rel_candidates_types",
        "rel_candidates_modules", "rel_cds_candidates", "rel_cds_protoclusters", "rel_regions_types",
        "rel_as_domains_substrates", "smcog_hits", "profile_hits", "rel_modules_monomers",
        "view_sequence_gc_content", "view_sequence_lengths", "preload_taxa", "preload_monomers"
    ]
    ...
    sql_to_csv(input_sql_dir / "preload_taxa.sql", taxa_csv, headers = ['tax_id', 'ncbi_taxid', 'superkingdom', 'kingdom', 'phylum', 'class', 'taxonomic_order', 'family', 'genus', 'species', 'strain', 'name'])
    sql_to_csv(input_sql_dir / "preload_monomers.sql", monomer_csv, headers =['monomer_id', 'substrate_id', 'name', 'description'])


    # Define exceptions for specific tables or views
    duckdb_exceptions = {"view_sequence_gc_content" : view_sequence_gc_content,
    ...
    for t in TABLES:
        ...
        if sql_file_path.is_file():
            ...
            sql_command = sqlparse.format(sql_command, reindent=False, keyword_case='upper')
            sanitized_sql = convert_postgres_to_duckdb(sql_command)
            # Split the sanitized SQL into individual statements
            statements = [str(statement).strip() for statement in sqlparse.parse(sanitized_sql) if str(statement).strip()]
            ...
            for num, statement in enumerate(statements):
                ...
                if statement.strip():
                    ...
                    try:
                        ...
                        logging.error(f"Failed to execute DuckDB SQL statement:\n{statement}\nError: {e}")
                        try:
                            for n, s in enumerate(statement.split(";")):
                                logging.debug(f"Executing SQL statement {num}.{n}:\n{s}\n")
                                conn.execute(s)
                        except Exception as e:
                            exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Initialize DuckDB schema from SQL files.")
    parser.add_argument("input_sql_dir", help="Directory containing input SQL files")
    parser.add_argument("output_dir", help="Directory to store output SQL files and DuckDB database file")
